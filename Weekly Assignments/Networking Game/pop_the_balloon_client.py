...
def run_game():
    ...
    while (keepGoing):
        ...
        if you_have_a_message(UDP_sock):
           ...
           split_received = received_string.split(" ")

           if "position" in split_received:
               b_x = float(split_received[1])
               b_y = float(split_received[2])
               b_xv = float(split_received[3])
               b_yv = float(split_received[4])

           elif "hit" in split_received:
               score = split_received[1]
               pop_sound.play()
