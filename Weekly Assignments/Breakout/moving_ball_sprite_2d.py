...
class MovingBall (Ball):
    ...
    def bounce_wall (self, width, height):
        ...
        elif self.p.y < 0+self.r:
            self.p.y = self.r
            self.v.y *= -1


    def hit_bottom(self, height):
        if self.p.y > (height - 4)-self.r:
            self.p.x = X_DEFAULT
            self.p.y = Y_DEFAULT
            return True
        return False
    ...
    def collideRect(self, rectangle):
        ...
        if abs_dist_bottom < self.r and rectangle.p.x <= self.p.x <= (rectangle.p.x + rectangle.width):
            ...
            return ""
        elif abs_dist_top < self.r and rectangle.p.x <= self.p.x <= (rectangle.p.x + rectangle.width):
            merge = self.r + (rectangle.height / 2) - abs_dist_top
            self.p.y -= merge
            self.reverseDirectionY()
            return ""
        elif abs_dist_left < self.r and rectangle.p.y <= self.p.y <= (rectangle.p.y + rectangle.height):
            self.reverseDirectionX()
            return ""
        elif abs_dist_right < self.r and rectangle.p.y <= self.p.y <= (rectangle.p.y + rectangle.height):
            self.reverseDirectionX()
            return ""
        return None
